		...
		<tbody>
			<c:forEach varStatus="i" var="b" items="${bills }">
				<tr>
					...
					<td>${b.address }</td>
					<td>${b.staffCreate }</td>
					<td><a type="button" class="btn btn-info" href="account/bill-detail-${b.id }.htm">Chi tiết</a></td>
					<td>
					<c:if test="${b.status eq 'N'.charAt(0) }"><a type="button" class="btn btn-danger" href="account/bill-${b.id }.htm">Hủy đơn</a></c:if>
					</td>
				</tr>
			</c:forEach>
		</tbody>
